...
const API_KEY = '5e0085ff488840a19e21e7ce9742a1e7'
...
getNews = async (event) => {
  ...
  const url = `
    https://newsapi.org/v2/everything?q=${search.value}&from=2019-01-01&sortBy=publishedAt&language=en&apiKey=${API_KEY}`
  const api_call = await fetch(url)
  const data = await api_call.json()
  console.log(data)
  if(data.cod !== '404' && search ){
    this.setState({
